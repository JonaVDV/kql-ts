 ...
 */
export type GetKeyFromQueryOrExtra<
	T extends { __extra: any },
	K,
> = K extends keyof T ? T[K] : T["__extra"];
/**
 * used to check if a value of a key is a boolean if it is, we return true, otherwise false
 */
export type IsKeyBooleanType<
 ...
 */
export type GetQueryTypeDefaultOrExtra<TQuery> = TQuery extends {
...
export type FunctionToReturn<T> = T extends (...args: any) => any
	? ReturnType<T>
	: T;
/**
...
export type KQLQueryTypeResolver<T extends KQLQuery> = T extends {
	select: infer S;
}
	? WrapIfCollection<
			{
				[K in keyof T["select"]]: CompareAndGetFromQuery<
					T["select"],
					K,
					T["query"]
				>;
